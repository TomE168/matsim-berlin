...
public class MainModeHandlerWithFilter implements TransitDriverStartsEventHandler, PersonDepartureEventHandler, ActivityEndEventHandler {
    ...
    List<Id<Person>> a100PersonIds;
    List<String> a100Links = new ArrayList<>();
    public MainModeHandlerWithFilter(List<Id<Person>> personIds) {
       a100PersonIds = personIds;
    }
    ...
    public Map<Id<Person>, List<String>> getPersonTrips() {
        ...
        for (Map.Entry<Id<Person>, List<String>> person : personTrips.entrySet()){
            if(a100PersonIds.contains(person.getKey())) {
                personTripsFiltered.put(person.getKey(), person.getValue());
            }
        }
        return personTripsFiltered;
    }
